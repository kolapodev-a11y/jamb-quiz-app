...
// Updated to handle new JSON format with passages
// ============================================
...
async function loadQuestions() {
    ...
    try {
        for (const subject of selectedSubjects) {
            ...
            if (allQuestionObjects.length === 0) {
                console.warn(`⚠️ ${subject}.json has no questions!`);
                continue;
            }

            // Flatten questions: handle both passage and independent types
            const flattenedQuestions = [];
            allQuestionObjects.forEach(qObj => {
                if (qObj.type === 'passage') {
                    // Passage with multiple sub-questions
                    qObj.questions.forEach(subQ => {
                    ...
                    // Standard format (no type specified) - treat as independent
                    flattenedQuestions.push({
                        questionNumber: qObj.questionNumber || qObj.id,
                        question: qObj.question,
                        options: qObj.options,
                        answer: qObj.answer,
                        explanation: qObj.explanation,
            ...
            const selectedQuestions = getRandomQuestions(flattenedQuestions, questionsCount);
            quizData.push(...selectedQuestions);
        }

        document.getElementById('totalQuestions').textContent = quizData.length;
        console.log(`✅ Loaded ${quizData.length} questions total`);
    } catch (error) {
...
function displayQuestion() {
    ...
    document.getElementById('questionSubject').textContent = question.subjectDisplay;

    // Display passage if present
    const passageContainer = document.getElementById('passageContainer');
    if (question.type === 'passage' && question.passage) {
...
// ============================================
// Submit Quiz (UPDATED for letter answers)
// ============================================
...
function submitQuiz() {
    ...
    quizData.forEach((question, index) => {
        ...
        const correctAnswerLetter = question.answer; // e.g., "A", "B", "C", "D"
        const correctAnswerIndex = correctAnswerLetter.charCodeAt(0) - 65; // Convert A->0, B->1, etc.
        ...
        detailedResults.push({
            ...
            userAnswer: userAnswerIndex !== null ? question.options[userAnswerIndex] : 'Not answered',
    ...
    // Update stats
    updateStats(scorePercentage);

    // Display results
    displayResults(scorePercentage, correctCount, detailedResults);
}

function displayResults(scorePercentage, correctCount, detailedResults) {
    showScreen('result-screen');

    document.getElementById('finalScore').textContent = `${scorePercentage}%`;
    document.getElementById('correctAnswers').textContent = correctCount;
    document.getElementById('totalQuestions2').textContent = quizData.length;

    // Detailed breakdown
    const breakdown = document.getElementById('detailedBreakdown');
    breakdown.innerHTML = detailedResults.map((result, index) => `
        <div class="result-item ${result.isCorrect ? 'correct' : 'incorrect'}">
            <div class="result-header">
                <strong>Q${result.questionNumber}</strong> - ${result.subject}
                <span class="result-badge">${result.isCorrect ? '✓ Correct' : '✗ Wrong'}</span>
            </div>
            ${result.passage ? `<div class="result-passage"><em>Passage excerpt: ${result.passage.substring(0, 100)}...</em></div>` : ''}
            <div class="result-question">${result.question}</div>
            <div class="result-answers">
                <div>Your answer: <strong>${result.userAnswer}</strong></div>
                ${!result.isCorrect ? `<div>Correct answer: <strong class="correct-answer">${result.correctAnswer}</strong></div>` : ''}
                ${result.explanation ? `<div class="explanation"><em>💡 ${result.explanation}</em></div>` : ''}
            </div>
        </div>
    `).join('');
}
